...
def sarimax_forecast(series, steps=4):
    """
    Train a SARIMAX model on a 1D series and forecast `steps` points.
...
# ============= MAIN FORECAST FUNCTION (CLI USE) =============
def forecast_sales(df, product_input):
    ...
    plt.title(
        f"Short Term Sales Forecast for Product: {pdf['description'].iloc[0]} using SARIMAX model",
        fontsize=14
    )
    plt.xlabel("Week")
    plt.ylabel("Sales")
